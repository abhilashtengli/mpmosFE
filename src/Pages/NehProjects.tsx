    ...
    const fetchProjects = async () => {
      try {
        ...
        const response = await axios.get(
          `${Base_Url}/get-all-neh-project-details`
        );
        ...
        if (result.success && result.data) {
          ...
          throw new Error("Failed to fetch NEH projects data");
        }
      } catch (err) {
        setError(
          err instanceof Error
            ? err.message
            : "An error occurred while fetching NEH projects"
        );
        console.error("Error fetching NEH projects:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="flex min-h-screen items-center justify-center p-4">
        <Alert className="max-w-md">
          ...
          <AlertDescription>
            <strong>Error loading NEH projects:</strong> {error}
          </AlertDescription>
        </Alert>
      </div>
    );
          ...
          {loading ? (
            ...
            >
              ...
              <p className="mx-auto mb-4 max-w-2xl text-base text-white font-medium drop-shadow-sm">
                Millet promotion initiatives across the North Eastern Hilly
                Region
              </p>
            </motion.div>
          )}
